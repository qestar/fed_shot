...
def init_nets(vocab, net_configs, n_parties, args, device='cpu'):
    ...
    if args.mode == 'few-shot' and args.method == 'new':
        for net_i in range(n_parties):
            ...
            net = LSTMAtt(ebd, args.out_dim, n_classes, total_classes, args)
            if device == 'cpu':
                net.to(device)
            else:
                net = net.cuda()
...
if __name__ == '__main__':
    ...
    fine_split_train_map = {class_: i for i, class_ in enumerate(list(range(2)))}
    ...
    X_train, y_train, X_test, y_test, net_dataidx_map, traindata_cls_counts, vocab = partition_data(
    ...
    nets, local_model_meta_data, layer_type = init_nets(vocab, args.net_config, args.n_parties, args, device='gpu')

    global_models, global_model_meta_data, global_layer_type = init_nets(vocab, args.net_config, 1, args, device='gpu')
